...
class AzureBlobStorageClient:
    def __init__(self):
        ...
        if os.getenv('BLOB_ACCOUNT_KEY'):
            ...
            self.blob_service_client: BlobServiceClient = BlobServiceClient(account_url=f"https://{self.account_name}.blob.core.windows.net/", credential=ManagedIdentityCredential(client_id=os.getenv("APP_CONTAINER_CLIENT_ID")))
    ...
    def upload_file(self, bytes_data, file_name: str, container_name: str, content_type: str):
        ...
        sas = None
        if self.account_key:
            sas = generate_blob_sas(account_name=self.account_name or "", container_name=container_name, blob_name=file_name,
                                account_key=self.account_key or "", permission="r", expiry=datetime.utcnow() + timedelta(hours=3))
        else:
            sas = generate_blob_sas(account_name=self.account_name or "", container_name=container_name, blob_name=file_name,
                                user_delegation_key=self.blob_service_client.get_user_delegation_key(datetime.utcnow(), datetime.utcnow() + timedelta(hours=1)), permission="r", expiry=datetime.utcnow() + timedelta(hours=3))
        return blob_client.url + '?' + sas

    def download_blob_to_bytes(self, container_name, blob_name):
        blob_client = self.blob_service_client.get_blob_client(
    ...
    def get_all_files(self, container_name: str):
        ...
        if self.account_key:
            ...
            sas = generate_container_sas(self.account_name,
                                     container_name,
                                     user_delegation_key=self.blob_service_client.get_user_delegation_key(datetime.utcnow(), datetime.utcnow() + timedelta(hours=1)),
                                     permission="r",
                                     expiry=datetime.utcnow() + timedelta(hours=3))
        files = []
    ...
    def get_all_urls(self, container_name: str, prefix: str = None):

        container_client = self.blob_service_client.get_container_client(
            container_name)
        blob_list = container_client.list_blobs(
            include='metadata', prefix=prefix or '')
    ...
    def upsert_blob_metadata(self, file_name, container_name, metadata):
        blob_client = BlobServiceClient.from_connection_string(
            self.connect_str).get_blob_client(container=container_name, blob=file_name)
