...
try:
    ...
    if user.get_role() != 'Admin':
        ...
        with st.expander("Caricamento CV", expanded=False):
            st.markdown(
                "In questa sezione è possibile caricare uno o più CV")

            uploaded_cv = st.file_uploader("Caricamento nuovi documenti",
                                           type=['txt', 'pdf'],
                                           key=3,
            ...
            if st.button("Carica CVs", key=4):
                if uploaded_cv:
                    ...
                    st.error("Devi selezionare almeno un file prima di poterlo caricare!")
